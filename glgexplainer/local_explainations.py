...
bamultishapes_classes_names = ["house", "grid", "wheel", "ba", "house+grid",
                               "house+wheel", "wheel+grid", "all"]
...
##
def read_bamultishapes(explainer="PGExplainer", dataset="BAMultiShapes", model="GCN",
                       split="TRAIN", evaluate_method=False, remove_mix=False,
                       min_num_include=5, manual_cut=None):
    ...
    for split in [split]:
        ...
        for c in ["1", "0"]:
            for pp in os.listdir(path + c + "/"):
                ...
                cut = elbow_method(np.triu(adj).flatten(), index_stopped,
                                   min_num_include) if manual_cut is None else manual_cut
                masked = copy.deepcopy(adj)
                masked[masked <= cut] = 0
                masked[masked > cut] = 1
...
# read local explanations and cut them based on a certain policy
##
def read_mutagenicity(explainer="PGExplainer", model="GCN_TF", split="TRAIN",
                      evaluate_method=True, min_num_include=None, manual_cut=None):
    ...
    for split in [split]:
        ...
        for c in ["0", "1"]:
            for pp in os.listdir(path + c + "/"):
                ...
                if len(connected_components) == adj.shape[
                    0]:  # only single nodes as connected component. No edges in the graph
                    continue
                else:
                    added = 0
                    for cc in connected_components:
                        if len(cc) >= 2:  # to exclude single nodes
                            ...
                            if not pattern_found:
                                le_classes.append(2)
                            if le_classes[
                                -1] == 0:  # temprary remove class 0 since there is just 1 example
                                del le_classes[-1]
                                continue
...
# read local explanations and cut them based on a certain policy
##
def read_hin(explainer="PGExplainer", model="GCN", split="TRAIN", min_num_include=2,
             manual_cut=None, priori_annotation=True):
    ...
    for split in [split]:
        ...
        for c in ["0", "1"]:
            for pp in os.listdir(path + c + "/"):
                ...
                if manual_cut is None:
                    cut = elbow_method(np.triu(adj).flatten(), index_stopped,
                                       min_num_include=min_num_include)
                else:
                    cut = manual_cut
