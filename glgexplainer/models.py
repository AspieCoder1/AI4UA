...
class GLGExplainer(torch.nn.Module):
    ...
    def __init__(self, len_model, le_model, device, hyper_params, classes_names,
        ...
        self.losses_names = ["loss", "prototype_distance_loss", "r1_loss", "r2_loss",
                             "concept_entropy_loss", "distribution_entropy_loss",
                             "div_loss", "len_loss", "debug_loss", "logic_loss"]

        self.optimizer = torch.optim.Adam(le_model.parameters(),
                                          lr=self.hyper["le_emb_lr"])
    ...
    def get_concept_vector(self, loader, return_raw=False):
        ...
        le_assignments = utils.prototype_assignement(self.hyper["assign_func"],
                                                     le_embeddings,
                                                     self.prototype_vectors, temp=1)
    ...
    def train_epoch(self, loader, train=True):
        ...
        total_losses = {k: torch.tensor(0., device=self.device) for k in
                        self.losses_names}
        preds, trues = torch.tensor([], device=self.device), torch.tensor([],
                                                                          device=self.device)
        le_classes = torch.tensor([], device=self.device)
        ...
        for data in loader:
            ...
            new_belonging = torch.tensor(utils.normalize_belonging(data.graph_id),
                                         dtype=torch.long, device=self.device)
            y = scatter(data.task_y, new_belonging, dim=0, reduce="max")
            y_train_1h = torch.nn.functional.one_hot(y.long(),
                                                     num_classes=self.num_classes).float().to(
                self.device)

            prototype_assignements = utils.prototype_assignement(
                self.hyper["assign_func"], le_embeddings, self.prototype_vectors,
                temp=1)
            total_prototype_assignements = torch.cat(
                [total_prototype_assignements, prototype_assignements], dim=0)
            le_classes = torch.concat([le_classes, data.y], dim=0)
            concept_vector = scatter(prototype_assignements, new_belonging, dim=0,
                                     reduce="max")

            loss, y_pred = self.compute_losses(le_embeddings, prototype_assignements,
                                               total_losses, concept_vector, y_train_1h,
                                               data.y)
        ...
        if self.hyper["debug_prototypes"]:
            ...
            acc_per_class = accuracy_score(trues.argmax(-1).cpu(),
                                           preds.argmax(-1).cpu())
            acc_overall = sum(
                trues[:, :].eq(preds[:, :] > 0).sum(1) == self.num_classes) / len(
                preds)  # it checks that the LEN predicted only one class. acc_per_class instead consider a sample correct even if the LEN fires both classes
        ...
        metrics["concept_vector_entropy"] = utils.entropy_loss(
            prototype_assignements).detach().cpu()  # change to total_prototype_assignements for the full information (more expensive)
        metrics["prototype_assignements"] = wandb.Histogram(
            prototype_assignements.detach().cpu())
    ...
    def debug_prototypes(self, le_embeddings, prototype_assignements, y):
        ...
        sample_prototype_distance = torch.cdist(le_embeddings, self.prototype_vectors,
                                                p=2) ** 2  # num_sample x num_prototypes
    ...
    def iterate(self, train_loader, val_loader, name_wandb="", config_wandb=None,
                save_metrics=True, plot=False):
        if self.hyper["log_wandb"]:
            self.run = wandb.init(
    ...
    def inspect(self, loader, log_wandb=False, plot=True, is_train_set=False):
        ...
        with torch.no_grad():
            ...
            if plot:  # plot embedding
                ...
                emb2d = emb if self.prototype_vectors.shape[
                                   1] == 2 else pca.fit_transform(emb)  # emb
                ...
                proto_2d = self.prototype_vectors.cpu().numpy() if \
                self.prototype_vectors.shape[1] == 2 else pca.transform(
                    self.prototype_vectors.cpu().numpy())
                plt.scatter(proto_2d[:, 0], proto_2d[:, 1], marker="x", s=60, c="black")
                for i, txt in enumerate(range(proto_2d.shape[0])):
                    plt.annotate("p" + str(i),
                ...
                for c in range(self.prototype_vectors.shape[0]):
                    plt.scatter(emb2d[concept_predictions == c, 0],
                                emb2d[concept_predictions == c, 1], label="p" + str(c))
            ...
            explanation0, explanation_raw = entropy.explain_class(self.len_model,
                                                                  x_train, y_train_1h,
                                                                  train_mask=torch.arange(
                                                                      x_train.shape[
                                                                          0]).long(),
                                                                  ...
                                                                  target_class=0,
                                                                  max_accuracy=True,
                                                                  topk_explanations=3000,
                                                                  try_all=False)
            accuracy0, preds = test_explanation(explanation0, x_train, y_train_1h,
                                                target_class=0, mask=torch.arange(
            ...
            explanation1, explanation_raw = entropy.explain_class(self.len_model,
                                                                  ...
                                                                  topk_explanations=3000,
                                                                  try_all=False)
            accuracy1, preds = test_explanation(explanation1, x_train, y_train_1h,
            ...
            if self.num_classes == 3:
                explanation2, explanation_raw = entropy.explain_class(self.len_model,
                                                                      ...
                                                                      topk_explanations=3000,
                                                                      try_all=False)
                accuracy2, preds = test_explanation(explanation2, x_train, y_train_1h,
                ...
                accuracy, preds = test_explanations([explanation0, explanation1],
                                                    x_train, y_train_1h,
                                                    mask=torch.arange(
                                                        x_train.shape[0]).long(),
                                                    material=False)
    ...
    def compute_losses(self, le_embeddings, prototype_assignements, total_losses,
        ...
        if self.hyper["debug_prototypes"]:
            loss, r1_loss, r2_loss, debug_loss = self.debug_prototypes(le_embeddings,
                                                                       prototype_assignements,
                                                                       le_y)
        ...
        # preds = torch.cat([preds, y_pred], dim=0)
        # trues = torch.cat([trues, y_train_1h], dim=0)
        len_loss = 0.5 * self.loss_len(y_pred, y_train_1h, self.hyper["focal_gamma"],
                                       self.hyper["focal_alpha"])
        ...
        if self.hyper["coeff_r1"] > 0:
            sample_prototype_distance = torch.cdist(le_embeddings,
        ...
        if self.hyper["coeff_logic_loss"] > 0:
            logic_loss = self.hyper[
                             "coeff_logic_loss"] * te.nn.functional.entropy_logic_loss(
                self.len_model)
        ...
        if self.hyper["coeff_pdist"] > 0:
            prototype_distances = torch.clip(
                utils.pairwise_dist(self.prototype_vectors), max=0.5).fill_diagonal_(
                float("inf"))
            prototype_distances = prototype_distances.min(-1).values
            prototype_distance_loss = self.hyper["coeff_pdist"] * - torch.mean(
                prototype_distances)
        ...
        if self.hyper["coeff_divloss"] > 0:
            ...
            cos_distances = torch.mm(proto_norm, torch.t(proto_norm)) - torch.eye(
                proto_norm.shape[0]).to(self.device) - 0.2
            matrix2 = torch.zeros(cos_distances.shape).to(self.device)
            div_loss = self.hyper["coeff_divloss"] * torch.sum(
                torch.where(cos_distances > 0, cos_distances, matrix2))
        ...
        if self.hyper["coeff_ce"] > 0:
            concept_entropy_loss = self.hyper["coeff_ce"] * utils.entropy_loss(
                prototype_assignements)
        ...
        if self.hyper["coeff_de"] > 0:
            ...
            total_losses[
                "distribution_entropy_loss"] += distribution_entropy_loss.detach()
        else:
...
class LEEmbedder(torch.nn.Module):
    ...
    def __init__(self, num_features, activation, num_gnn_hidden=20, dropout=0.1,
                 num_hidden=10, num_layers=2, backbone="GIN"):
        super().__init__()

        if backbone == "GIN":
            nns = torch.nn.ModuleList([
                torch.nn.Sequential(
                    torch.nn.Linear(num_features if i == 0 else num_gnn_hidden,
                                    num_gnn_hidden),
                    torch.nn.LeakyReLU(),
            ...
            self.convs = torch.nn.ModuleList([
                GATv2Conv(num_features if i == 0 else num_gnn_hidden,
                          int(num_gnn_hidden / 4), heads=4) for i in range(num_layers)
            ])
        elif backbone == "SAGE":
            self.convs = torch.nn.ModuleList([
                SAGEConv(num_features if i == 0 else num_gnn_hidden, num_gnn_hidden) for
                i in range(num_layers)
            ])
        elif backbone == "SAGE_sum":
            self.convs = torch.nn.ModuleList([
                SAGEConv(num_features if i == 0 else num_gnn_hidden, num_gnn_hidden,
                         aggr="add") for i in range(num_layers)
            ])
        elif backbone == "GCN":
            self.convs = torch.nn.ModuleList([
                GCNConv(num_features if i == 0 else num_gnn_hidden, num_gnn_hidden) for
                i in range(num_layers)
            ])
        elif backbone == "GraphConv":
            self.convs = torch.nn.ModuleList([
                GraphConv(num_features if i == 0 else num_gnn_hidden, num_gnn_hidden)
                for i in range(num_layers)
            ])
        else:
...
def LEN(input_shape, temperature, n_classes=2, remove_attention=False):
    layers = [
        te.nn.EntropyLinear(input_shape, 10, n_classes=n_classes,
                            temperature=temperature, remove_attention=remove_attention),
        torch.nn.LeakyReLU(),
