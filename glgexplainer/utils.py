...
class LocalExplanationsDataset(InMemoryDataset):
    ...
    def __init__(self, root, adjs, feature_type, belonging, y=None, task_y=None,
                 precomputed_embeddings=None, transform=None, pre_transform=None,
                 pre_filter=None):
        ...
        for i, adj in enumerate(adjs):
            ...
            data = Data(x=features,
                        ...
                        y=torch.tensor(int(y[i]),
                                       dtype=torch.long) if y is not None else None,
                        # the type of local explanation
                        task_y=torch.tensor(int(task_y[belonging[i]]),
                                            dtype=torch.long) if y is not None else None,
                        # the class of the original input graph
...
class GroupBatchSampler():
    ...
    def __iter__(self):
        batch = []
        num_graphs_added = 0
        graph_idxs = random.sample(np.unique(self.belonging).tolist(),
                                   len(np.unique(self.belonging)))
...
def build_dataloader(dataset, belonging, num_input_graphs):
    batch_sampler = GroupBatchSampler(num_input_graphs=num_input_graphs,
                                      drop_last=False, belonging=np.array(belonging))
...
def pairwise_dist(embeddings, squared=False):
    ...
    dist = square_norm.unsqueeze(0) - 2 * dot_prod + square_norm.unsqueeze(
        1)  # Pairwise distances
    dist = dist.clamp(
        min=0)  # Some values might be negative due to numerical instability. Set distences to >=0
    if not squared:
        ...
        dist = dist * (
                    1 - mask)  # Correct the epsilon added: set the distances on the mask to be 0
    return dist
...
def prototype_assignement(assign_func, le_embeddings, prototype_vectors, temp):
    ...
    if assign_func == "softmax*10":
        le_assignments = F.softmax(-torch.nn.functional.normalize(
            torch.cdist(le_embeddings, prototype_vectors, p=2)) * 10, dim=-1)
    elif assign_func == "1/x":
        le_assignments = inverse(
            torch.cdist(le_embeddings, prototype_vectors, p=2))  # 1/x
...
def plot_molecule(data, adj=None, node_features=None, composite_plot=False):
    if adj is None and node_features is None:
        ...
        G = nx.from_numpy_matrix(adj)
    node_label = data.x.argmax(-1) if node_features is None else node_features.argmax(
        -1)
    max_label = node_label.max() + 1
    nmb_nodes = len(node_label)

    colors = ['orange', 'red', 'lime', 'green', 'lightseagreen', 'orchid', 'darksalmon',
              'darkslategray', 'gold', 'bisque', 'tan', 'blue', 'indigo', 'navy']
    color_to_atom = {
        'orange': "C",
        'red': "O",
        'lime': "Cl",
    ...
    for i in range(max_label):
        ...
        nx.draw_networkx_labels(G, pos,
                                {k: color_to_atom[colors[i]] for k in node_filter})
...
def plot_etn(data, ax=None):
    ...
    nx.draw_networkx_labels(G, pos, {k: v for k, v in convert_hin_labels(G).items() if
                                     k in G.nodes()}, font_size=18, font_color="black",
                            ax=ax)